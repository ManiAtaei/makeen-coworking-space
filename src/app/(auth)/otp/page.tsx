...
export default function Otp() {
  ...
  const onSubmit: SubmitHandler<type> = () => {
    setIsTimerRunning(true);
    // Optionally, use verificationCode for submission logic
  ...
  const handleCodeChange = (
    ...
    if (e.target.value && index < 4) { // Adjusted to 4 since you have 5 inputs
  ...
  return (
    <div className="pl-7 pr-5 max-w-[600px] mx-auto flex flex-col justify-center h-screen">
      ...
      <div className="flex flex-col items-center mt-[-15px]">
        <Image
          src="/login/imageLogin/logo-makeen.svg"
          alt="Makeen Logo"
              ...
              <div className="flex flex-row mt-5 gap-[9px] mobileNum:gap-[16px]">
                {verificationCode.map((item, index) => (
                  <input
                    type="text"
                    key={index}
                    id={`code-${index}`} // Add id for focus handling
                    maxLength={1}
                    value={item} // Bind input value to state
